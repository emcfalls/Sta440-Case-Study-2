    ...
    for (k in (j+1):length(all_fullnames)) {
      for (l in (k+1):length(all_fullnames)) {
        for (m in (l+1):length(all_fullnames)) {
          ...
          hb_result <- sum(subset(mHB_Top_Three, FullName %in% combo)$avg_result)
          fx_result <- sum(subset(mFX_Top_Three, FullName %in% combo)$avg_result)
          pb_result <- sum(subset(mPB_Top_Three, FullName %in% combo)$avg_result)
          ph_result <- sum(subset(mPH_Top_Three, FullName %in% combo)$avg_result)
          sr_result <- sum(subset(mSR_Top_Three, FullName %in% combo)$avg_result)
          vt_result <- sum(subset(mVT_Top_Three, FullName %in% combo)$avg_result)
          allaround_result <- sum(subset(mAllAround_Top_Three, FullName %in% combo)$avg_result)
