...
## LOG INTO https://hypatia.stat.duke.edu AND THEN CLONE OUR REPO THERE AND THEN RUN THESE SIMULATION .QMD FILES
...
## THE SET-UP BEFORE THE ALGO -- make sure to replace read.csv with the correct file path
```{r}
## Re-arranged data
...
estimateKernelDensity <- function(outputOfCreateProperDataframe, bandwidthValue){
  ...
  if (apparatusOfInterestHere == "BB"){
    if (howManyScoresWeHaveForThatAthleteXApparatusCombination < 4){
      ...
      nugget = sd(outputOfCreateProperDataframe[[1]])*4
    }
  }
