...
sims601_700 <- c()
  for (i in 601:700){
    sims601_700 <- c(sims601_700, sprintf("SimulationResults_%s", i))
  }

sims701_800 <- c()
  for (i in 701:800){
    sims701_800 <- c(sims701_800, sprintf("SimulationResults_%s", i))
  }

sims801_900 <- c()
  for (i in 801:900){
    sims801_900 <- c(sims801_900, sprintf("SimulationResults_%s", i))
  }

sims901_1000 <- c()
  for (i in 901:1000){
    sims901_1000 <- c(sims901_1000, sprintf("SimulationResults_%s", i))
  }
...
data_601 <- all %>%
  filter(iteration %in% sims601_700)

data_701 <- all %>%
  filter(iteration %in% sims701_800)

data_801 <- all %>%
  filter(iteration %in% sims801_900)

data_901 <- all %>%
  filter(iteration %in% sims901_1000)
...
all_team_scores <- get_team_scores(601, 700, hb, fx, pb, ph, sr, vt)
```

```{r}
# example of result from sim1
all_team_scores$sim601
```


```{r}
# run and save sim results from USA to csv under folder "simulaiton results"
df <- get_sim_results(601, 700, all_team_scores, "USA")

write.csv(df, "simulation_data_elyse/mens_simulation_601_to_700.csv") # Young
```
