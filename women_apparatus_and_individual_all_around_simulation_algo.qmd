...
all_fullnames <- unique(c(aBB_Top_Three$FullName, aFX_Top_Three$FullName, aUB_Top_Three$FullName, aVT_Top_Three$FullName, aAllAround_Top_Three$FullName))

all_fullnames
```

## Finding the best number of medals/combination of athletes
    ...
    for (k in (j+1):length(all_fullnames)) {
      for (l in (k+1):length(all_fullnames)) {
        for (m in (l+1):length(all_fullnames)) {
          ...
          # Filter the dataframes for the specific FullNames
          bb_result <- sum(head(subset(aBB_Top_Three, FullName %in% combo)[order(-subset(aBB_Top_Three, FullName %in% combo)$avg_result), ]$avg_result, 2))
          fx_result <- sum(head(subset(aFX_Top_Three, FullName %in% combo)[order(-subset(aFX_Top_Three, FullName %in% combo)$avg_result), ]$avg_result, 2))
          ub_result <- sum(head(subset(aUB_Top_Three, FullName %in% combo)[order(-subset(aUB_Top_Three, FullName %in% combo)$avg_result), ]$avg_result, 2))
          vt_result <- sum(head(subset(aVT_Top_Three, FullName %in% combo)[order(-subset(aVT_Top_Three, FullName %in% combo)$avg_result), ]$avg_result, 2))
          allaround_result <- sum(head(subset(aAllAround_Top_Three, FullName %in% combo)[order(-subset(aAllAround_Top_Three, FullName %in% combo)$avg_result), ]$avg_result, 2))
...
result_df$Total_Result <- rowSums(result_df[, c("BB_Result", "FX_Result", "UB_Result", "VT_Result", "AllAround_Result")])
