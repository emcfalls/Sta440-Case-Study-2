    ...
    for (k in (j+1):length(all_fullnames)) {
      for (l in (k+1):length(all_fullnames)) {
        for (m in (l+1):length(all_fullnames)) {
          ...
          bb_result <- sum(subset(aBB_Top_Three, FullName %in% combo)$avg_result)
          fx_result <- sum(subset(aFX_Top_Three, FullName %in% combo)$avg_result)
          ub_result <- sum(subset(aUB_Top_Three, FullName %in% combo)$avg_result)
          vt_result <- sum(subset(aVT_Top_Three, FullName %in% combo)$avg_result)
          allaround_result <- sum(subset(aAllAround_Top_Three, FullName %in% combo)$avg_result)
