...
convert_simulation_results_to_numeric_list <- function(data, column_name = "SimulationResults") {
  # Function to convert a comma-separated string to a numeric list
  convert_string_to_numeric_vector <- function(data_string) {
    numeric_vector <- as.numeric(unlist(strsplit(data_string, ", ")))
    return(numeric_vector)
  }

  # Apply the conversion function to each row in the specified column
  data$NumericList <- lapply(data[[column_name]], convert_string_to_numeric_list)

  return(data)
}

aVT <- read.csv("VTResults.csv") 
aBH <- convert_simulation_results_to_numeric_list(aBH, "SimulationResults")


aFX <- read.csv("simluation result/women_FX_1000.csv")
aUB <- read.csv("simluation result/women_UB_1000.csv")
aVT <- read.csv("simluation result/women_VT_1000.csv")

## calculating the top three potential people in each individual event 

aBB$SimulationResults[1]
...
write.csv(all_aroud_finalists, file = "women_all_aroud_finalists_1000.csv")



write.csv(aBB_prepared, file = "women_BB_1000.csv")
write.csv(aUB_prepared, file = "women_UB_1000.csv")
write.csv(aFX_prepared, file = "women_FX_1000.csv")
write.csv(aVT_prepared, file = "women_VT_1000.csv")
