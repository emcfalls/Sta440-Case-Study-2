...
get_team_scores <- function(iter, samples){
  ...
  df_bb_eligible <- df_bb_eligible %>%
    ...
    pivot_longer(cols = SimulationResults_1:SimulationResults_2, names_to = "iteration")
  ...
  for (i in 1:iter){
    ...
    # 4e. add simulation to list
    all_team_scores[[sprintf("sim%s", i)]] <- olympics_data
...
```{r}
all_team_scores <- get_team_scores(5, test_samples)
```
...
### 3. Finding Candidates (top 5 for each apparatus by Country)


```{r}
all_sims <- candidates(5, all, bb_all, ub_all, fx_all, vt_all)
```

```{r}
olympics1 <- all_sims$sim1
head(olympics1)
```
...
```{r}
sum(usa_bb_score,  usa_ub_score,  usa_fx_score,  usa_vt_score)
```
