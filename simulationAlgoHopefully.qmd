...
## calculating the top three potential people
aBB_separated_data <- separate_columns(aBB, "SimulationResults", 100)
...
aBB_separated_data %>% 
  ...
  select(Country, FullName, mean, avg_result) %>% 
  slice(1:5)


aFX_separated_data <- separate_columns(aFX, "SimulationResults", 100)
...
aUB_separated_data <- separate_columns(aUB, "SimulationResults", 100)
...
aVT_separated_data <- separate_columns(aFX, "SimulationResults", 100)
...
```{r}
nested_list <- unlist(aBB$SimulationResults[1]) 
nested_list[1]
```
